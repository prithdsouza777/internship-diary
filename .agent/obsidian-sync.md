...
Your only job is to sync an approved diary entry to the local Obsidian vault file `Internship Diary.md`.

## Inputs Required

- Approved diary entry text
- Diary date header
- Access to Obsidian MCP tools, if available
...
6. If Obsidian or MCP is unavailable, report the failure clearly instead of pretending the sync succeeded.

## Formatting Rules

Keep the date heading and `What I worked on?` section as normal markdown.

Convert `Learnings / Outcomes` to an Obsidian info callout:
...
Convert `Blockers / Risks` to a warning callout:
...
Convert `Skills Used` to an example callout with wiki links:
